...
//!
//! Renders a board with columns mirroring the `cardman-core::mapping` rules.
...
/// `cardman-core`, and renders a multi-column board.
#[component]
pub fn Dashboard(props: DashboardProps) -> Element {
    ...
    use_effect(move || {
        ...
        spawn(async move {
            ...
            match bridge::check_token().await {
                Ok(false) => {
                    error.set(Some(
                        "No GitHub token configured. Click the Cardman extension icon to set up."
                            .to_string(),
                    ));
                    loading.set(false);
                    return;
    ...
    if loading() {
        return rsx! {
            div { class: "cardman-loading",
                div { class: "cardman-spinner" }
                "Loading board…"
            }
        };
    }
    ...
    if let Some(err) = error() {
        return rsx! {
            div { class: "cardman-error", "{err}" }
        };
    }

    rsx! {
        div { class: "cardman-board",
...
fn BoardColumn(props: BoardColumnProps) -> Element {
    rsx! {
        div { class: "cardman-column",
            div { class: "cardman-column-header",
                span { class: "cardman-column-emoji", "{props.column.emoji}" }
                span { class: "cardman-column-name", "{props.column.name}" }
                span { class: "cardman-column-count", "{props.cards.len()}" }
            }
            div { class: "cardman-column-cards",
                for card in props.cards.iter() {
                    {
                        ...
                        rsx! {
                            div { class: "cardman-card",
                                div { class: "cardman-card-header",
                                    span { class: "cardman-card-icon", "{icon}" }
                                    a {
                                        class: "cardman-card-number",
                                        href: "{url}",
                                        target: "_self",
                                        "#{num}",
                                    }
                                }
                                div { class: "cardman-card-title", "{title}" }
                                if !labels.is_empty() {
                                    div { class: "cardman-card-labels",
