// Cardman extension — content script loader.
//
// Runs on GitHub pages, injecting the Cardman dashboard tab and PR issue
// linker. Uses URL /issues#cardman to toggle the Kanban view inline.
...
(function () {
    ...
    /** Inject the Cardman tab aligned with other GitHub tabs. */
    function injectDashboardTab(page) {
        if (document.getElementById("cardman-tab")) return;
        ...
        tab.setAttribute("data-tab-item", "cardman");
        tab.innerHTML =
            ...
            '</svg>' +
            ' <span data-content="Cardman">Cardman</span>';
        ...
        tab.addEventListener("click", (e) => {
            ...
            if (location.pathname.replace(/\/$/, "") === issuesPath) {
                ...
                location.assign(issuesPath + "#cardman");
            }
        });
        ...
        navBody.querySelectorAll("a").forEach((ghTab) => {
            if (ghTab.id === "cardman-tab") return;
            ghTab.addEventListener("click", () => {
                if (dashboardOpen) closeDashboard();
            });
        });

        // Auto-open if URL has #cardman hash (e.g. navigated from another page)
        if (location.hash === "#cardman") {
    ...
    function openDashboard(page) {
        const tab = document.getElementById("cardman-tab");
        if (!tab || dashboardOpen) return;

        // CSS attribute hides GitHub content containers
        document.body.setAttribute("data-cardman-open", "");

        // Mark Cardman tab as active
        tab.setAttribute("aria-current", "page");

        // Create or show the dashboard container (inline, under the tabs)
        let container = document.getElementById("cardman-dashboard");
        if (!container) {
            container = document.createElement("div");
            container.id = "cardman-dashboard";
        ...
        container.innerHTML =
            '<div class="cardman-loading"><div class="cardman-spinner"></div>Loading board…</div>';

        if (location.hash !== "#cardman") {
            history.replaceState(null, "", `${location.pathname}#cardman`);
        }
        dashboardOpen = true;
    ...
    function closeDashboard() {
        ...
        document.body.removeAttribute("data-cardman-open");

        const tab = document.getElementById("cardman-tab");
        if (tab) tab.removeAttribute("aria-current");

        const container = document.getElementById("cardman-dashboard");
    ...
    async function loadDashboard(container, page) {
        try {
            ...
            if (!hasToken?.hasToken) {
                container.innerHTML =
                    '<div class="cardman-error">No GitHub token configured. Click the Cardman extension icon to set up.</div>';
                return;
            }
            ...
            container.innerHTML = `<div class="cardman-error">${err.message}</div>`;
        }
    }
    ...
    function renderDashboard(container, page) {
        ...
        layout.className = "cardman-layout";

        // ── Left sidebar filter panel ──
        const sidebar = document.createElement("div");
        sidebar.className = "cardman-sidebar";
        ...
        if (isOrgContext) {
            ...
            if (classifiedData.orgs.length > 1) {
                ...
                orgSelect.className = "cardman-org-select";
                ...
                orgLabel.className = "cardman-sidebar-title";
        ...
        boardArea.className = "cardman-board-area";
        const board = document.createElement("div");
        board.className = "cardman-board";
        board.id = "cardman-board";
    ...
    function renderRepoCheckboxes(sidebar, repos, container, page) {
        ...
        title.className = "cardman-sidebar-title";
        title.textContent = "Repositories";
        sidebar.appendChild(title);

        const list = document.createElement("div");
        list.className = "cardman-repo-list";

        repos.forEach((r) => {
            const label = document.createElement("label");
            label.className = "cardman-repo-item";
    ...
    async function loadBoard() {
        const boardEl = document.getElementById("cardman-board");
        if (!boardEl) return;

        if (selectedRepos.length === 0) {
            boardEl.innerHTML =
                '<div class="cardman-hint">Select one or more repositories</div>';
            return;
        }

        boardEl.innerHTML =
            '<div class="cardman-loading"><div class="cardman-spinner"></div>Loading…</div>';
        ...
        try {
            ...
            const results = await Promise.all(
                selectedRepos.map((r) =>
                    bgMessage({ type: "fetchCards", owner: r.owner, repo: r.repo })
                        ...
                        .catch((err) => {
                            console.warn(`[Cardman] Failed to fetch ${r.owner}/${r.repo}:`, err.message);
            ...
            boardEl.innerHTML = `<div class="cardman-error">${err.message}</div>`;
        }
    }
    ...
    function renderBoard(boardEl, columns, cards) {
        ...
        columns.forEach((colDef) => {
            ...
            col.className = "cardman-column";

            const hdr = document.createElement("div");
            hdr.className = "cardman-column-header";
            hdr.innerHTML = `
        <span class="cardman-column-emoji">${colDef.emoji}</span>
        <span class="cardman-column-name">${colDef.name}</span>
        <span class="cardman-column-count">${colCards.length}</span>`;
            col.appendChild(hdr);

            const cardList = document.createElement("div");
            cardList.className = "cardman-column-cards";
            ...
            colCards.forEach((c) => {
                ...
                card.className = "cardman-card";
                card.href = url;
                card.target = "_blank";
                card.rel = "noopener";

                const repoTag = multiRepo
                    ? `<span class="cardman-card-repo">${c.owner}/${c.repo}</span>`
                    : "";

                let assigneeHtml = "";
                if (c.assignees && c.assignees.length > 0) {
                    assigneeHtml = '<div class="cardman-card-assignees">';
                    c.assignees.forEach((a) => {
                        if (a.avatar_url) {
                            assigneeHtml += `<img class="cardman-card-avatar" src="${a.avatar_url}&s=20" alt="${a.login}" title="${a.login}">`;
                        } else {
                            assigneeHtml += `<span class="cardman-card-avatar-fallback" title="${a.login}">${(a.login || "?").charAt(0).toUpperCase()}</span>`;
                        }
                    });
                    assigneeHtml += "</div>";
                }
                ...
                if (c.labels && c.labels.length > 0) {
                    labelsHtml = '<div class="cardman-card-labels">';
                    c.labels.forEach((l) => {
                        labelsHtml += `<span class="cardman-label" style="background:#${l.color}">${l.name}</span>`;
                    });
                    labelsHtml += "</div>";
                }

                card.innerHTML = `
          <div class="cardman-card-header">
            <span class="cardman-card-icon">${icon}</span>
            <span class="cardman-card-number">#${c.number}</span>
            ${repoTag}
          </div>
          <div class="cardman-card-title">${c.title}</div>
          ${assigneeHtml}
          ${labelsHtml}`;
    ...
    function injectIssueLinker(page) {
        if (document.getElementById("cardman-linker-widget")) return;
        ...
        widget.id = "cardman-linker-widget";
        widget.className = "cardman-linker-widget";

        // Collapsible header
        const header = document.createElement("div");
        header.className = "cardman-linker-header";
        header.innerHTML = "🃏 Link Issues";
        header.addEventListener("click", () => {
            body.style.display = body.style.display === "none" ? "" : "none";
            header.classList.toggle("cardman-linker-header-open");
        });

        const body = document.createElement("div");
        body.className = "cardman-linker-body";
        body.style.display = "none";

        // Repo dropdown
        const repoRow = document.createElement("div");
        repoRow.className = "cardman-linker-row";
        repoRow.innerHTML = "<label>Repository</label>";
        const repoSelect = document.createElement("select");
        repoSelect.className = "cardman-input";
        ...
        searchRow.className = "cardman-linker-row";
        searchRow.innerHTML = "<label>Search issues</label>";
        const searchInput = document.createElement("input");
        searchInput.type = "text";
        searchInput.className = "cardman-input";
        ...
        searchBtnRow.className = "cardman-linker-row";
        const searchBtn = document.createElement("button");
        searchBtn.className = "cardman-btn cardman-btn-primary";
        ...
        results.className = "cardman-search-results";
        results.innerHTML =
            '<p class="cardman-hint">Type a query and click Search</p>';
        body.appendChild(results);

        // Selected list
        const selectedRow = document.createElement("div");
        selectedRow.className = "cardman-linker-row";
        selectedRow.innerHTML = "<label>Selected issues</label>";
        const selectedList = document.createElement("div");
        selectedList.className = "cardman-selected-issues";
        selectedList.innerHTML =
            '<p class="cardman-hint">No issues selected</p>';
        selectedRow.appendChild(selectedList);
        body.appendChild(selectedRow);

        // Action buttons
        const actionRow = document.createElement("div");
        actionRow.className = "cardman-linker-actions";
        const addBtn = document.createElement("button");
        addBtn.className = "cardman-btn cardman-btn-primary";
        ...
        function doSearch() {
            ...
            bgMessage({
                ...
                .then((data) => {
                    ...
                    if (items.length === 0) {
                        results.innerHTML =
                            '<p class="cardman-hint">No results found</p>';
                        return;
                    }
                    items.forEach((item) => {
                        ...
                        row.className = "cardman-search-item";
                        row.innerHTML = `
              <span class="cardman-search-number">#${item.number}</span>
              <span class="cardman-search-title">${item.title}</span>`;
                        const btn = document.createElement("button");
                        btn.className = "cardman-btn-add";
                ...
                .catch((err) => {
                    results.innerHTML = `<p class="cardman-error-sm">${err.message}</p>`;
                })
                .finally(() => {
                    searchBtn.textContent = "Search";
        ...
        function updateSelectedDisplay() {
            ...
            selected.forEach((s, i) => {
                const row = document.createElement("div");
                row.className = "cardman-selected-item";
                row.innerHTML = `<span>Issue: ${s.repo}#${s.number}</span>`;
                const rm = document.createElement("button");
                rm.className = "cardman-btn-remove";
                rm.textContent = "✕";
        ...
        addBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                errP.className = "cardman-error-sm";
    ...
    function cleanup() {
        dashboardOpen = false;
        document.body.removeAttribute("data-cardman-open");
        ["cardman-tab", "cardman-dashboard", "cardman-linker-widget"].forEach(
            (id) => {
