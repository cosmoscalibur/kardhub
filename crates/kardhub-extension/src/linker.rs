        ...
        if dialog_open() {
            div {
                ...
                div {
                    ...
                    // Body
                    div { class: "kardhub-dialog-body",
                        // Repo picker
                        div { class: "kardhub-linker-row",
                            label { r#for: "kardhub-repo-select", "Repository" }
                            input {
                                id: "kardhub-repo-select",
                                class: "kardhub-input",
                                r#type: "text",
                                value: "{target_repo}",
                        ...
                        // Search input
                        div { class: "kardhub-linker-row",
                            label { r#for: "kardhub-issue-search", "Search issues" }
                            input {
                                id: "kardhub-issue-search",
                                class: "kardhub-input",
                                r#type: "text",
                        ...
                        div { class: "kardhub-linker-row",
                            button {
                                class: "kardhub-btn kardhub-btn-primary",
                                disabled: searching(),
                        ...
                        // Results
                        div { class: "kardhub-search-results",
                            if search_results().is_empty() && !searching() {
                                p { class: "kardhub-hint", "Start typing and click Search" }
                            }
                            for (num, title) in search_results().iter() {
                                {
                                    ...
                                    let target = target_repo();
                                    rsx! {
                                        div { class: "kardhub-search-item",
                                            span { class: "kardhub-search-number", "#{num}" }
                                            span { class: "kardhub-search-title", "{title}" }
                                            button {
                                                class: "kardhub-btn-add",
                                                disabled: already,
                                                onclick: move |_| {
                        ...
                        div { class: "kardhub-linker-row",
                            ...
                            div { class: "kardhub-selected-issues",
                                if selected_issues().is_empty() {
                                    p { class: "kardhub-hint", "No issues selected" }
                                }
                                for (i, issue) in selected_issues().iter().enumerate() {
                                    div { class: "kardhub-selected-item",
                                        span { "Issue: {issue.repo}#{issue.number}" }
                                        button {
                                            class: "kardhub-btn-remove",
                                            onclick: move |_| {
                                                let mut current = selected_issues();
                                                current.remove(i);
                                                selected_issues.set(current);
                    ...
                    // Footer
                    div { class: "kardhub-dialog-footer",
                        if let Some(err) = apply_error() {
                            p { class: "kardhub-error-sm", "{err}" }
                        }
                        button {
                            class: "kardhub-btn kardhub-btn-primary",
